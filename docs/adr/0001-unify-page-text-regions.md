...
- Page Commands use the Page Revision as an optimistic-concurrency boundary; stale commands fail instead of overwriting newer region state. Commands are serialized per project because page edits also update project-level override maps and manifests; a failed commit restores the prior page document and project manifest, removes snapshots created by the failed batch, and rebuilds the project index.
