...
- 前端新增任务连接生命周期模块，集中拥有 WebSocket、预期关闭、断线重连、后台任务恢复订阅和停止任务请求；`AppV2.vue` 只保留任务完成后的项目刷新和图片副作用。
...
本轮新增验证包括：`task-event-state` 纯函数测试、`useTranslationTaskConnection` 连接生命周期测试、既有 `workflow-state` 测试、设置持久化测试、前端生产构建，以及完整工作台浏览器 E2E。
