...
- [ ] 定义 Schema v2 与 v1 → v2 迁移。
- [ ] 先在加载/保存边缘把旧 dict 转换为模型，Engine 内部暂时可通过 facade 使用。
- [ ] 项目索引改为可从 manifest 重建。
- [ ] 加入损坏 JSON、未知版本、部分缺失产物测试。

完成定义：持久化入口不再接受任意 dict；新旧项目都能恢复且产物状态可解释。
