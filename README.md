...
- NVIDIA GPU 不要求刻意升级到最新驱动；启动脚本会在显卡支持的前提下，按当前
  驱动自动选择兼容的 PyTorch/CUDA 运行时

首次启动会安装较大的机器学习依赖。Windows CUDA 运行时约 2 GB，终端会显示
下载进度；解包、安装等无法计算百分比的静默阶段每 15 秒显示一次“仍在运行”和
累计用时。脚本会对 PyTorch 官方源和阿里云镜像做小流量测速，优先使用较快的
...
显卡会使用支持 Blackwell 的 CUDA 运行时：R580 及以上使用 CUDA 13，R570–R579
自动使用 CUDA 12.8 兼容组合。只有当前驱动确实无法支持显卡运行时才需要升级。

**新前端一键启动（Windows）**：双击 `start-v3.bat` 即可完成同样的一键流程
（依赖安装 + 后端 + 新前端 InkStage），并自动打开独立浏览器窗口
（`http://127.0.0.1:5273/`，端口被占时自动顺延）。想手动跑时：
`start.bat v3` 等价于 `start-v3.bat`；不带参数运行 `start.bat` 仍启动旧前端。
...
- `fonts/system` 是项目随附的开源预置字体源；启动时会复制到 `.runtime/fonts/system` 使用。
- 自定义字体统一存放在 `.runtime/fonts/custom`。旧版 `fonts/custom` 会在迁移时兼容复制。
- 应用只读取这两个目录，不扫描操作系统字体。
- “打开字体文件夹”会打开 `.runtime/fonts`。
- 项目、输出、设置、模型、字体、Electron 数据、临时文件、缓存和日志默认统一保存在项目根目录的 `.runtime/`，不会提交到 Git。设置面板会显示并可直接打开当前存储根目录；如有需要，可用 `APP_DATA_DIR` 指定另一个统一根目录。
- 后端日志会持续追加并自动轮转；设置面板可以直接打开日志目录或导出脱敏诊断包。
- 需要让可信的 Codex 在另一台局域网电脑上直接运行 CUDA 测试时，可在设置中一次性启用“持久远程任务节点”；协议和操作边界见[远程任务节点说明](docs/remote-execution.md)。

升级后会提示迁移旧仓库与 Windows AppData 中的数据。可选择保留旧目录，或在复制并验证全部旧项目后只清理本应用拥有的旧目录；可能与其他软件共用的 Hugging Face/Torch 全局缓存不会自动删除。完整规则见[运行时存储说明](docs/runtime-storage.md)。

## 安全与隐私

- 后端默认只监听 `127.0.0.1`。
- 持久远程任务节点默认关闭；用户主动启用后才监听局域网地址，并使用独立的固定 Token 保护上传、命令执行、日志和结果接口。
...
不必一律升级到 R580：例如 577 驱动会自动安装 PyTorch 2.11 + CUDA 12.8，
R580 及以上会安装 PyTorch 2.12 + CUDA 13。可在 PowerShell 中检查：
...
下载完成后的解包和安装可能暂时没有百分比，此时每 15 秒会显示“仍在运行”和
累计用时。单个连接连续 30 秒没有数据会重试或换源；只要下载仍在持续接收数据，
就不会因为总耗时较长而中止。详细记录位于：
